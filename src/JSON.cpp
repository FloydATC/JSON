...
JSON::JSON()
{
  ...
  sstream << "null";
  this->root = std::make_shared<JSON_node>(&sstream);
}
...
{
  this->root = std::make_shared<JSON_node>(&istream);
}
...
JSON_nodeptr JSON::getNode(std::string path)
{
  JSON_nodeptr parent_node = this->root;
...
{
  if (path == "") {
    this->root = std::make_shared<JSON_node>(node);
  } else {
    ...
    JSON_nodeptr parent_node = this->getNode(parent_path);
...
void JSON::setBoolean(std::string path, bool value) 
{
  ...
  this->setNode(path, JSON_node(&sstream) );
}


void JSON::setNumber(std::string path, double value) 
{
  this->setNode(path, JSON_node(value) );
}
...
void JSON_node::parse_array()
{
  ...
  this->array_value = JSON_arraytype();
  ...
  while(this->peek() != ']') {
    ...
    JSON_nodeptr child = std::make_shared<JSON_node>(this->istream);

    // Add item to self
    this->array_value.push_back(child);
...
void JSON_node::parse_object()
{
  ...
  this->object_value = JSON_objecttype();
  ...
  while(this->peek() != '}') {
    ...
    JSON_nodeptr value = std::make_shared<JSON_node>(this->istream);

    // Add key/value pair to self
    this->object_value.insert({ key, value });
...
void JSON_node::parse_string()
{
  ...
  this->string_value = this->find_quoted_string();
}
...
void JSON_node::parse_value()
{
  ...
  if (this->is_digit() || this->peek() == '-') {
    ...
    // Convert to double
    size_t offset = 0;
    this->number_value = std::stod(string, &offset);
    return;
  } else {
...
{
  size_t count = this->array_value.size();
  size_t index = 0;
  if (count > 0) {
    os << "[" << std::endl;

    for (auto& value : this->array_value) {
...
{
  size_t count = this->object_value.size();
  size_t index = 0;
  if (count > 0) {
    os << "{" << std::endl;
    ...
    for (auto& pair : this->object_value) keys.push_back(pair.first);
    ...
    for (auto& key : keys) {
      this->dump_prefix(os, depth+1);
      os << "\"" << key << "\": ";
      JSON_nodeptr child = this->object_value.at(key);
...
void JSON_node::dump(std::ostream& os, uint8_t depth) const
{
  switch(this->node_type) {
    ...
    case JSON_nodetype::JSON_nodetype_boolean: 
      os << (this->boolean_value ? "true" : "false");
      break;
    case JSON_nodetype::JSON_nodetype_number: 
      os << std::to_string(this->number_value);
      break;
...
JSON_nodeptr JSON_node::getChildByIndex(size_t index)
{
  if (this->node_type != JSON_nodetype::JSON_nodetype_array) return nullptr;
  if (index >= this->array_value.size()) return nullptr;
  return this->array_value.at(index);
}


JSON_nodeptr JSON_node::getChildByKey(std::string key)
{
  if (this->node_type != JSON_nodetype::JSON_nodetype_object) return nullptr;
  if (this->object_value.count(key) == 0) return nullptr;
  return this->object_value.at(key);
}
...
void JSON_node::setChildByIndex(size_t index, JSON_node node)
{
  ...
  this->array_value.at(index) = std::make_shared<JSON_node>(node);
}
...
void JSON_node::setChildByKey(std::string key, JSON_node node)
{
  ...
  this->object_value.insert( {key, std::make_shared<JSON_node>(node)} );
}
...
void JSON_node::addChildNode(JSON_node node)
{
  ...
  this->array_value.push_back(std::make_shared<JSON_node>(node));
}
